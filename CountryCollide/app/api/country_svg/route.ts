...
export async function GET(req: Request) {
  try {
    ...
    const isoToNum = await loadIsoToNum(); // build-time JSON, no fetch()
    const findByISO = makeCountryFinderISO(features, isoToNum);

    const fa = findByISO(a);
    if (!fa) {
      return new Response(
        JSON.stringify({ error: `Country not found`, missing: { a: true } }),
    ...
    mA = centerAndScale(mA, width, height, 16);
    ...
    if (!aPath && !debug) {
      return new Response(
        JSON.stringify({
          error:
            'Union failed for these inputs. Try debug=1 to inspect outlines.',
        }),
